...
import './ManageTutorialAdd.css'
import ManageDndTable from './ManageDndTable'
import { Radio } from 'antd'
import AddTags from './tags'
export default class ManageTutorialAdd extends React.Component{
 state = {
    value: 1,
  };
	onChange = e => {
	...
	render(){
		...
		<Grid item md={4} xs={4}><Labelbox type="text" labelname="Tutorial Title"/></Grid>
		<Grid item md={4} xs={4}><Labelbox type="text" labelname="Author"/></Grid>
		<Grid item md={2} xs={2}><Labelbox type="text" labelname="Created Date"/></Grid>
		...
		<Grid item md={2} xs={6}><Labelbox type="text" labelname="Cost"/></Grid>
		<Grid item md={3} xs={6} className="active_radio">
		 <div><label className="active">Discount Applicable</label></div>
		<Radio.Group onChange={this.onChange} value={this.state.value}>
        <Radio value={1}>Cost</Radio>
        <Radio value={2}>Percentage</Radio>
      
      </Radio.Group>
   
		</Grid>
		<Grid item md={2} xs={6}><Labelbox type="text" labelname="Cost Discount"/></Grid>
		      <Grid item md={5} xs={6}>
